...
def ProcessNote(midiEvent, currentEventTickValue,currentTrackNumber):
    ...
    #Note off events: update the durations of notes as they expire
        ...
        try:
            ...
            for x in enumerate(TickToPitchMidiValueDictionary[tickValueOfActiveNote]):
                (entryIndex, testPitchMidiValueEntry) = (x[0],x[1])
...
def PreprocessTracks(midiTracks,maximumNumberOfTracks):
...
def BuildTickToPitchMidiValueDictionary(midiTracks,maximumNumberOfTracks):
    ...
    minNoteForAllTracks,maxNoteForAllTracks,averageOfAllTracks = PreprocessTracks(midiTracks,maximumNumberOfTracks)
